...
export default function ListView({ cfg, slug }) {
  ...
  const cellValue = (row, col) => {
    ...
    if (col.f === "pill") {
      ...
      } else if (col.f === "image") {          // <-- add this block here
      content = raw ? (
        <img
          src={raw}
          alt=""
        ...
        <span className="flex h-9 w-9 items-center justify-center rounded border border-line bg-[#f5f8fd]">
          <Icon name="image" size={14} className="text-cell" />
        </span>
      );
    } else if (col.f === "dash") {
