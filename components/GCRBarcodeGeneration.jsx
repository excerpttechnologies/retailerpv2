    ...
    try {
      const res = await fetch("/api/barcode-generation", {
        ...
        body: JSON.stringify({
          ...
          totals, // aggregates so the backend can build the GRC header
        }),
      });
      ...
      setRows([]);
      setLookupMsg("");
      // refresh the saved-records table below so the newly submitted rows show up
      fetchSaved(1);
      // optionally: redirect to the GRC list
              ...
              <tr className="bg-amber-50">
                ...
                <td className={tdBase}>
                  <input
                    ...
                    onKeyDown={(e) => e.key === "Enter" && focusNext(refs.disc)}
                  />
                </td>
                <td className={tdBase}>
                  <input
