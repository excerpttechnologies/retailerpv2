...
/* /api/sell-b2binvoice - list + create. */
...
export async function GET(req) {
  ...
  const total = await B2bInvoice.countDocuments(filter);
  const rows = await B2bInvoice.find(filter)
    .sort({ createdAt: -1 })
    .skip((page - 1) * perPage)
    .limit(perPage)
    .lean();
...
export async function POST(req) {
  ...
  const created = await B2bInvoice.create(doc);
