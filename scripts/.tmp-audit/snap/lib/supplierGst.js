/* Supplier GST uniqueness - the server side.
   ...
   Server-only: it queries the contact collection. The client-safe half -
...
import Contact, { SUPPLIER_GST_INDEX } from '@/models/Contact';
...
export async function findSupplierGstConflict({ gstNo, businessId, excludeId }) {
  ...
  return Contact.findOne(filter, { businessName: 1, shortName: 1, contactId: 1 })
    .collation(COLLATION)
    .lean();
}
