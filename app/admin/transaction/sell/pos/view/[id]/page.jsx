...
export default function ViewPosPage() {
  ...
  return (
    <div className="card p-5">
      <div className="mb-4 flex items-center justify-between"><h1 className="text-lg font-bold">View POS</h1><button className="btn" onClick={() => router.back()}>Back</button></div>
      ...
      <div className="mt-4 overflow-x-auto"><table className="dt"><thead><tr>{['Item Code', 'Item Name', 'HSN', 'GST (%)', 'Quantity', 'RSP', 'Discount', 'Tax', 'Subtotal'].map((x) => <th key={x}>{x}</th>)}</tr></thead><tbody>{(doc.items || []).map((item, i) => <tr key={i}><td>{item.code || item.itemCode || '-'}</td><td>{item.description || item.name || '-'}</td><td>{item.hsn || '-'}</td><td>{money(item.gst)}</td><td>{item.qty || 0}</td><td>{money(item.rsp)}</td><td>{money(item.discountAmount || Number(item.rsp || 0) * Number(item.qty || 0) * Number(item.discountPct || 0) / 100)}</td><td>{money(Number(item.lineTotal || 0) * Number(item.gst || 0) / 100)}</td><td>{money(item.lineTotal)}</td></tr>)}</tbody><tfoot><tr><td colSpan="8" className="text-right font-bold">Total Payable</td><td className="font-bold">{money(doc.totalAmount)}</td></tr><tr><td colSpan="8" className="text-right font-bold">Total Paid</td><td>{money(doc.paid)}</td></tr><tr><td colSpan="8" className="text-right font-bold">Total Remaining</td><td>{money(doc.sellDue)}</td></tr></tfoot></table></div>
