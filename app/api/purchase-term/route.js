...
export async function GET(req) {
  ...
  const total = await PurchaseTerm.countDocuments(filter);
  const rows = await PurchaseTerm.find(filter)
    .sort({ createdAt: -1 })
    .skip((page - 1) * perPage)
    .limit(perPage)
    .lean();
...
export async function POST(req) {
  ...
  const created = await PurchaseTerm.create(doc);
