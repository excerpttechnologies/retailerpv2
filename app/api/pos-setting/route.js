...
import { FIELDS } from '@/app/admin/setting/pos_setting/fields';

/* /api/pos-setting - single document per scope: read + upsert. */
...
export async function GET(req) {
  ...
  const doc = await PosSetting.findOne(scopeOf(sp)).lean();
...
export async function POST(req) {
  ...
  await PosSetting.findOneAndUpdate(scopeOf(sp), doc, { upsert: true, new: true });
