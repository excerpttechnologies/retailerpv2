...
export async function GET(req) {
  ...
  const total = await CityGroup.countDocuments(filter);
  const rows = await CityGroup.find(filter)
    .sort({ createdAt: -1 })
    .skip((page - 1) * perPage)
    .limit(perPage)
    .lean();
...
export async function POST(req) {
  ...
  const created = await CityGroup.create(doc);
