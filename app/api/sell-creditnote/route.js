...
import Upstream from '@/models/SalesReturn';
import { requireSession } from '@/lib/session';
import { resolveRefLabels } from '@/lib/refLabels';
import { validate, escapeRegex } from '@/lib/validate';
import { nextDocNumber } from '@/lib/docnumber';
import { FORM } from '@/app/admin/transaction/sell/creditnote/form';
...
/* /api/sell-creditnote - list + create. */
...
export async function POST(req) {
  ...
  if (!doc.creditNoteCode) {
    doc.creditNoteCode = await nextDocNumber(CreditNote, 'creditNoteCode', "Credit Note", {
      businessId: doc.businessId, locationId: doc.locationId, finYear: doc.finYear,
    });
  }

  const created = await CreditNote.create(doc);
