...
export function toLabelData(row) {
  ...
  /* The stored barcode value - SUPPLIER_CODE * GRC_NUMBER * BILL_SL_NO * SEQ
     for a GRC barcode (lib/barcodeValue.js) - exactly as saved. The bars
     encode it and every line of the label that shows it shows this same
     string. */
  const barcode = String(r.barcodeNo || r.barcodeGenerated || r.itemCode || '');
  return {
    barcode,
    /* Separate fields for barcodeNo and barcodeGenerated to allow
       different positioning on the label (barcodeNo on LEFT, barcodeGenerated on RIGHT) */
    barcodeNo: String(r.barcodeNo || ''),
    barcodeGenerated: String(r.barcodeGenerated || ''),
