...
export function toGridRow(row, index) {
  return {
    ...
    uom: row.uom || '',
    /* THE UNIT'S OWN NUMBER FIRST, the composed value only as a fallback.

       These two fields are different things - barcodeNo is the number the
       till scans and the bars encode ("9A1077"), barcodeGenerated is the
       composed reference printed beside it ("G512 * 05173 * 1 * 1") - and
       this line used to hand the grid the composed value under BOTH names.
       The sticker is built from whichever row it is given, so the Print
       Label Picker preview drew the composed value on the left with an empty
       right-hand side and encoded it in the bars, while the Barcode Print
       page - which reads the same records straight from the API - drew
       "9A1077" on the left, the composed value on the right, and encoded the
       short number. One record, two different labels, depending on the
       screen. Reading barcodeNo first is the precedence the rest of the
       application already uses (unitBarcode below, labelKey in
       lib/barcodeLabelPrint.js), so preview and print now agree by
       construction. A row that only has a composed value still falls back to
       it and is unchanged. */
